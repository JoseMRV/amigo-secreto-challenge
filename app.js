...
function reinicio(){
    listaAmigos = [];
    asignarElementoHTML('h2', 'La lista ha sido reiniciada. ¡Agrega nuevos amigos para hacer el sorteo!');
...
function agregarNombre(){
    ...
    if (listaAmigos.includes(amigos)){
        asignarElementoHTML('h2', 'Este nombre ya esta registrado, escribe otro o escribelo de otra forma');
        limpiarCaja();
        return;
    }
    if(amigos === ''){
        asignarElementoHTML('h2', 'Lo siento, debes escribir un nombre');
        limpiarCaja();
        return;
    }
